...
# quando interagir com alguma coisa. A variável "player_animation" serve para #
# quando o personagem estiver de uma certa posição. Adicionei um tempo        #
...
func interacao():
	if Input.is_action_just_pressed("atirar") && (primeira_pagina == true) && interacao_nome_kinematicbody2D.is_colliding() && (player.animation == "Direita"):
